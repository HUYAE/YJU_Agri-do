...
export default function Home() {
  ...
  const handleClickExpand = (question: string) => {
    if (openedQuestions.includes(question)) {
      setOpenedQuestions(
          openedQuestions.filter(item => item !== question)
      )
    } else {
      // ... 为解构运算符，可以把一个数组打散
      ...
      <h1 className={styles.title}>FAQ</h1>
        <div className={styles.formbox}>
          <input
              ...
              placeholder={'Enter your question here...'} className={styles.textbox}/>
          <button
              onClick={handleSearch}
              className={styles.search}>Search</button>
          <button
              className={styles.ask}
              onClick={()=>{Router.push('./askpage')}}
          >  Ask</button>
        </div>
        {searched && answers.length === 0 && (
            <h3>No Results</h3>
        )}
                ...
                <li key={answer.question}>
                  <h3 className={styles.question}>
                    ...
                    <img
                        ...
                        src={DownIcon.src} width={30}/>
                  </h3>
                  {open && (
                      <ul className={styles.answerList}>
                        <li
                            onClick={()=>{Router.push('./answerpage')}}
                            className={styles.answerItem} key={'answer'}>
